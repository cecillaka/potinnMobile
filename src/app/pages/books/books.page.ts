...
getHeaders() {
 ...
 submitForm() {
  ...
  if ( this.form.get('files').value !== ' ' ) {
    ...
    this.http.post('http://127.0.0.1:8000/api/file/books' , formData, this.getHeaders()  ).subscribe(
        // check errors
      (response: any) => {
  ...
  getBooks() {
    ...
    this.http.get('http://127.0.0.1:8000/api/file/books', this.getHeaders()   ).subscribe((apart: any) => {
        this.imgProps = apart;
